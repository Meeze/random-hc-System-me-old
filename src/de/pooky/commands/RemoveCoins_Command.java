...
public class RemoveCoins_Command implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2, String[] args) {
		...
		if(p.hasPermission("coins.removecoins")) {
		if(args.length != 2) {
			p.sendMessage("§7Verwendung : §c/removecoins <Spieler> <Betrag>");
		}else {
			if(MySQL.isConnected()) {
				...
				Coin_API.removeCoins(t.getPlayer(), coins);
				p.sendMessage("§8» §6Dir wurden " + coins + "$" + " §6abgezogen.");
